...
export default function PetEvolutionScreen() {
  ...
  const [currentStage, setCurrentStage] = useState(2); // Rồng con
  ...
  const handleFeed = () => {
    ...
    setMood('happy');
    Alert.alert('🍖 Cho ăn thành công!', 'Rồng con đã được cho ăn! +30 XP\n\nHãy giữ streak 14 ngày để tiến hóa!');
  };

  const handleSimulateSick = () => {
    setMood('sick');
    Alert.alert('😷 Rồng con bị ốm!', 'Streak bị đứt đoạn! Rồng con của bạn bị ốm và có thể tụt cấp nếu không chăm sóc.\n\nHãy hoàn thành nhiệm vụ mỗi ngày để hồi phục!');
  };
  ...
  const handleSimulateEvolve = () => {
    if (currentStage < evolutionStages.length - 1) {
      ...
      setStreak(0);
      Alert.alert('🎊 TIẾN HÓA THÀNH CÔNG!', `Thú cưng đã tiến hóa thành ${evolutionStages[currentStage + 1].name} ${evolutionStages[currentStage + 1].emoji}!\n\nTiếp tục giữ streak để tiến hóa lên cấp tiếp theo!`);
    }
  };
    ...
    <ScrollView style={L.screen} contentContainerStyle={[L.content, { paddingTop: 50 }]} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.petCard}>
        <View style={styles.petSky}>
          ...
          {mood === 'sick' && <Text style={styles.sickBubble}>😷 Ốm rồi...</Text>}
          {mood === 'happy' && <Text style={styles.happyBubble}>😄 No rồi!</Text>}
        </View>
        <View style={styles.petInfo}>
          <View style={styles.petNameRow}>
            <Text style={styles.petName}>{stage.name}</Text>
            <View style={styles.levelBadge}><Text style={styles.levelText}>CẤP {stage.level}</Text></View>
          </View>
          <View style={styles.moodRow}>
            <Text style={styles.moodLabel}>Tâm trạng:</Text>
            <Text style={styles.moodValue}>{mood === 'happy' ? '😄 Vui vẻ' : mood === 'sick' ? '😷 Bị ốm' : '😐 Bình thường'}</Text>
          </View>
          <View style={styles.xpRow}>
            <Text style={styles.xpLabel}>XP: {currentXP.toLocaleString()}</Text>
            {nextStage && <Text style={styles.xpToNext}>Còn {xpToNext} XP để tiến hóa</Text>}
          </View>
          <View style={styles.xpBar}>
            <View style={[styles.xpFill, { width: `${Math.min(progress, 100)}%` }]} />
          </View>

          <Pressable style={[styles.feedBtn, fed && styles.feedBtnDone, mood === 'sick' && styles.feedBtnSick]} onPress={handleFeed}>
            <Ionicons name={fed ? 'checkmark-circle' : 'restaurant'} size={20} color={fed ? C.green : '#FFF'} />
            <Text style={[styles.feedBtnText, fed && { color: C.green }]}>{fed ? 'Đã cho ăn hôm nay' : 'Cho rồng ăn (30 XP)'}</Text>
          </Pressable>
        </View>
      </View>
      ...
      <Text style={styles.streakHint}>Giữ streak 14 ngày liên tiếp để tiến hóa! ({streak}/14)</Text>

      <View style={styles.streakGrid}>
        {streakHistory.map((day) => (
          <View key={day.day} style={[styles.streakDay, day.done ? styles.streakDone : day.day <= 14 ? styles.streakPending : null]}>
            <Text style={[styles.streakDayText, day.done && styles.streakDayTextDone]}>{day.day}</Text>
            ...
            return (
              <View key={evo.level} style={styles.evoStep}>
                <View style={styles.evoLeft}>
                  <View style={[styles.evoDot, isReached && styles.evoDotReached, isCurrent && styles.evoDotCurrent]}>
                    <Text style={styles.evoDotEmoji}>{evo.emoji}</Text>
                  </View>
                  {i < evolutionStages.length - 1 && (
                    <View style={[styles.evoLine, isReached && styles.evoLineReached]} />
                  )}
                </View>
                <View style={styles.evoRight}>
                  ...
                  {!isReached && <Text style={styles.evoLocked}>🔒 Chưa mở khóa</Text>}
                </View>
              </View>
            );
          })}
      ...
      <Text style={styles.demoHint}>Các nút bên dưới để demo flow cho giảng viên</Text>
      <View style={styles.demoRow}>
        <Pressable style={styles.demoBtn} onPress={handleSimulateEvolve}>
          <Text style={styles.demoBtnText}>🐉 Demo Tiến hóa</Text>
        </Pressable>
        <Pressable style={[styles.demoBtn, styles.demoBtnDanger]} onPress={handleSimulateSick}>
          <Text style={styles.demoBtnText}>😷 Demo Bị ốm</Text>
        </Pressable>
      </View>
      <Pressable style={[styles.demoBtn, { marginBottom: 20 }]} onPress={() => { setMood('normal'); setCurrentStage(1); setCurrentXP(1200); setStreak(11); setFed(false); }}>
        <Text style={styles.demoBtnText}>🔄 Reset Demo</Text>
      </Pressable>
...
const styles = StyleSheet.create({
  ...
  overline: { color: '#FDCB6E', fontSize: 10, fontWeight: '800', letterSpacing: 1.2 },
  ...
  petSky: { height: 220, backgroundColor: '#BFD6FF', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' },
  cloud1: { position: 'absolute', width: 120, height: 42, borderRadius: 25, backgroundColor: 'rgba(255,255,255,0.65)', top: 30, left: -30 },
  cloud2: { position: 'absolute', width: 100, height: 36, borderRadius: 22, backgroundColor: 'rgba(255,255,255,0.5)', top: 80, right: -25 },
  petEmoji: { fontSize: 100, zIndex: 2 },
  petShadow: { width: 110, height: 22, borderRadius: 50, backgroundColor: 'rgba(48,76,155,0.2)', position: 'absolute', bottom: 24 },
  sickBubble: { position: 'absolute', top: 35, right: 20, backgroundColor: '#FFF', borderRadius: 12, paddingHorizontal: 10, paddingVertical: 6, fontSize: 12 },
  happyBubble: { position: 'absolute', top: 35, right: 20, backgroundColor: '#FFF', borderRadius: 12, paddingHorizontal: 10, paddingVertical: 6, fontSize: 12 },
  petInfo: { padding: 18 },
  ...
  levelBadge: { backgroundColor: C.orangeSoft, borderRadius: 999, paddingHorizontal: 12, paddingVertical: 5 },
  ...
  xpBar: { height: 10, backgroundColor: '#EFF1F7', borderRadius: 5, overflow: 'hidden', marginBottom: 16 },
  xpFill: { height: '100%', backgroundColor: C.primary, borderRadius: 5 },
  feedBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, backgroundColor: C.primary, borderRadius: 14, height: 50 },
  ...
  streakBadge: { flexDirection: 'row', alignItems: 'center', gap: 4, backgroundColor: C.orangeSoft, borderRadius: 999, paddingHorizontal: 12, paddingVertical: 6 },
  ...
  streakGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 6, marginBottom: 22 },
  streakDay: { width: 40, height: 40, borderRadius: 10, backgroundColor: '#EFF1F7', alignItems: 'center', justifyContent: 'center' },
  streakDone: { backgroundColor: C.green },
  streakPending: { borderWidth: 1, borderColor: C.border, borderStyle: 'dashed' },
  ...
  evoDot: { width: 40, height: 40, borderRadius: 20, backgroundColor: '#EFF1F7', alignItems: 'center', justifyContent: 'center', zIndex: 2 },
  evoDotReached: { backgroundColor: C.primarySoft },
  evoDotCurrent: { backgroundColor: C.primary, borderWidth: 3, borderColor: '#B8C4FF' },
  ...
  demoBtn: { flex: 1, backgroundColor: C.primarySoft, borderRadius: 12, paddingVertical: 14, alignItems: 'center' },
