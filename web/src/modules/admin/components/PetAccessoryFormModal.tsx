...
export default function PetAccessoryFormModal({ isOpen, onClose, initialData, onSubmit }: PetAccessoryFormModalProps) {
  ...
  const [category, setCategory] = useState('Mũ');
  const [priceXP, setPriceXP] = useState(100);
  const [icon, setIcon] = useState('🎩'); // simple emoji placeholder for icon selection

  useEffect(() => {
    if (initialData) {
      setName(initialData.name || '');
      setCategory(initialData.category || 'Mũ');
      setPriceXP(initialData.priceXP || 100);
      setIcon(initialData.icon || '🎩');
    } else {
      setName(''); setCategory('Mũ'); setPriceXP(100); setIcon('🎩');
    }
  }, [initialData, isOpen]);
      ...
      <div className="admin-modal" style={{ width: 450 }}>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div>
            ...
            <input required type="text" value={name} onChange={e => setName(e.target.value)} style={{ width: '100%', padding: '8px 12px', borderRadius: 8, border: '1px solid var(--admin-border)' }} />
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
            <div>
              ...
              <select value={category} onChange={e => setCategory(e.target.value)} style={{ width: '100%', padding: '8px 12px', borderRadius: 8, border: '1px solid var(--admin-border)' }}>
                ...
                <option value="Vương miện">Vương miện</option>
