/**
 * Xoá nền ô caro "giả trong suốt" mà AI vẽ luôn vào ảnh → nền trong suốt thật.
 * Dùng chung cho ảnh pet (fix-backgrounds.mjs) và ảnh phụ kiện (fix-accessory-backgrounds.mjs).
 *
 ...
 * Nhờ vậy phần trắng của nhân vật/đồ vật không bị ăn nhầm như khi chỉ lọc theo màu.
 */
...
/**
 * Dò lưới ô caro từ góc trên-trái ảnh (data = RGBA raw).
 * Trả về màu ô sáng/tối + kích thước/pha ô theo 2 trục; ném lỗi nếu không thấy nền caro.
 */
export function detectChecker(data, w) {
  ...
  const lightParity = (Math.floor((2 - fx.phase) / fx.size) + Math.floor((2 - fy.phase) / fy.size)) & 1
    ? (lum(2 * w + 2) > mid ? 1 : 0)
    : (lum(2 * w + 2) > mid ? 0 : 1);
  return { light, dark, mid, fx, fy, lightParity };
}

/** Màu ô caro "đáng lẽ" ở (x, y); nearLine = sát đường kẻ ô (màu có thể là ô sáng hoặc tối) */
export function expectedChecker(grid, x, y) {
  const { fx, fy, light, dark, lightParity } = grid;
  const cx = (x - fx.phase) / fx.size, cy = (y - fy.phase) / fy.size;
  const nearLine = Math.abs(cx - Math.round(cx)) * fx.size < 2 || Math.abs(cy - Math.round(cy)) * fy.size < 2;
  const isLight = ((Math.floor(cx) + Math.floor(cy)) & 1) === lightParity;
  return { value: isLight ? light : dark, nearLine };
}

export async function fixImage(file) {
  ...
  const { light, dark, mid, fx, fy, lightParity } = detectChecker(data, w);
...
export async function hasRealTransparency(file) {
